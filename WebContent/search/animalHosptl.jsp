    ...
    <div class="container">
      ...
      <ul class="pagination justify-content-center">
        ...
        <c:choose>
		  ...
		  <c:otherwise>
		    <a class="page-link" href="<%=request.getContextPath()%>/search?cmd=animalhosptl&page=0">&laquo;</a>
		  </c:otherwise>
	    </c:choose>
	    </li>
        ...
        <c:choose>
		  ...
		  <c:otherwise>
		    <a class="page-link" href="<%=request.getContextPath()%>/search?cmd=animalhosptl&page=${param.page - 1}">&lt;</a>
		  </c:otherwise>
	    </c:choose>
	    </li>
	    ...
	    <c:if test="${param.page >= 4 && param.page < (lastPage - 5)}">
	      <c:forEach var="num" begin="0" end="7">
	      	<li class="page-item">
              <a class="page-link" href="<%=request.getContextPath()%>/search?cmd=animalhosptl&page=${param.page + num - 2}">${param.page + num - 1}</a>
	     	</li>
	      </c:forEach>
	    </c:if>
	    ...
	    <c:if test="${param.page >= (lastPage - 5)}">
	      <c:forEach var="num" begin="0" end="7">
	        <li class="page-item">
              <a class="page-link" href="<%=request.getContextPath()%>/search?cmd=animalhosptl&page=${lastPage - (7 - num)}">${lastPage - (6 - num)}</a>
	       	</li>
	      </c:forEach>
	    </c:if>
		...
		<c:if test="${param.page <= 3}">
		  <c:forEach var="num" begin="0" end="7">
		    <li class="page-item">
              <a class="page-link" href="<%=request.getContextPath()%>/search?cmd=animalhosptl&page=${num}">${num + 1}</a>
	        </li>
	      </c:forEach>
		</c:if>
        ...
        <c:choose>
		  ...
		  <c:otherwise>
		    <a class="page-link" href="<%=request.getContextPath()%>/search?cmd=animalhosptl&page=${lastPage}">&raquo;</a>
		  </c:otherwise>
	    </c:choose>
	    </li>   	  
	  </ul>
